...
moveTaskToCompleted(taskToUpdate) {
  ...
  // Inside your TaskManager class
completeTask(taskId) {
  const updateCompleted = this.tasks.findIndex((task) => task.id == taskId);
  const taskToUpdate = this.tasks[updateCompleted];

  if (taskToUpdate) {
    // Assuming moveTaskToCompleted is a method of your TaskManager class
    this.moveTaskToCompleted(taskToUpdate);
      const newId = Math.floor(Math.random() * 1000); // Generate a new unique ID
    taskToUpdate.id = newId;

    const description = typeof taskToUpdate.description === 'object'
